...
export default function SettingPopUp({ settingOn }) {
  ...
  const { setNameCash, setYearCash, setCampus, customBookmarkCount, increaseBookmark, decreaseBookmark } = useSettingStore();
  ...
  return (
    <>
      <div
        className={`setting-container ${settingClass}`}
        {...stopSwiperHandler}
      >
        ...
        <form action="#" style={{ display: "flex" }}>
          ...
          >
            ...
            {isOpen && (
              <ul className="input-number-options" {...stopSwiperHandler}>
                {numList.map((num, idx) => {
                  return (
                    <li
                      onClick={(e) => {
        ...
        <div className="setting-bookmark-container">
          <span>북마크 수</span>
          <div className="bookmark-count-control">
            <button className="bookmark-count-btn" onClick={decreaseBookmark}
              disabled={customBookmarkCount <= 4}>−</button>
            <span className="bookmark-count-num">{customBookmarkCount}개</span>
            <button className="bookmark-count-btn" onClick={increaseBookmark}
              disabled={customBookmarkCount >= 8}>+</button>
          </div>
        </div>
        <div className="setting-campus-container">
          <div className="setting-campus">캠퍼스 선택</div>
          <button
            className="campus-choose-btn"
